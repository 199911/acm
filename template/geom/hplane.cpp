...
void hpi() {
  ...
  REP(i, hn){
    ...
    if ( fl == 1 && ccw( hp[a[i]][0], hp[a[i]][0], lt) ) continue; 
    ...
    if ( fl == 0 && qh != qt && ( ( ( hp[q[qh]][1] - hp[q[qh]][0] ) ^ (hp[a[i]][1] - hp[a[i]][0]) ) < - EPS ) ) {
      fl = 1;
  ...
  rn = 0;
  lli( hp[q[qh]][0], hp[q[qh]][1], hp[q[qt-1]][0], hp[q[qt-1]][1], res[rn++]);
  FOR(i, pqh, pqt) {
    res[rn++] = pq[i];
  }
